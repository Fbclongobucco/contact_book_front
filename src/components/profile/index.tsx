...
export function Profile() {
    ...
    return (
        <div className="w-full  p-3 shadow-2xl bg-zinc-100 rounded-lg flex gap-20">
            <div className="w-2/4">
                <h2 className="text-lg text-cyan-950 font-bold">Nome: {user?.name}</h2>
                <h3 className="text-cyan-950 font-bold">Email: {user?.email}</h3>
            </div>
            <div className="w-2/4 text-lg font-bold text-cyan-950">
                <p>CPF: {user?.cpf}</p>
                <p>Aniversário: {user?.birthday}</p>
                <p>Criado em: {user?.createAt.slice(0,10)}</p>
            </div>
            <Link href="/edit-profile" className="bg-cyan-800 w-18 h-7 text-cyan-100 flex justify-center rounded">editar</Link>
        </div>
    )
}